...
protocol LeccoPlayerUpdateDelegate:NSObjectProtocol {
    //播放进度更新回调方法
...
class LeccoPlayerView: UIView {
    ...
    var delegate:LeccoPlayerUpdateDelegate?    //代理
    ...
    var playerLayer:AVPlayerLayer = AVPlayerLayer()          //视频播放器图形化载体
    ...
    func play() {
        LeccoPlayerManager.shared().play(player: player!)
    }
    
    func pause() {
        LeccoPlayerManager.shared().pause(player: player!)
    }
    
    func replay() {
        LeccoPlayerManager.shared().replay(player: player!)
    }
...
extension LeccoPlayerView: URLSessionTaskDelegate, URLSessionDataDelegate {
    //网络资源下载请求获得响应
...
extension LeccoPlayerView: AVAssetResourceLoaderDelegate {
