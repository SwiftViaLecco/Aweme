...
class LeccoLoadMoreControl: UIControl {
    
    var indicator: UIImageView = UIImageView(image: UIImage.init(named: "icon30WhiteSmall"))
    ...
    func initSubView() {
        ...
        label.font = UIFont.leccoFont(size: 10)
    ...
    override func layoutSubviews() {
        ...
        if edgeInsets == nil {
            ...
            edgeInsets?.bottom += (50.0 + kSafeAreaBottomHeight)
                                ...
                                if tableView.visibleCells.count > 0 {
                                    if let indexPath = tableView.indexPath(for: tableView.visibleCells.last!) {
                                        ...
                                        if indexPath.section == lastSection && indexPath.row == lastRow {
                                            self.frame = CGRect.init(x: 0, y: tableView.visibleCells.last?.frame.maxY ?? 0, width: kScreenWitdh, height: 50)
                                        }
                                    }
                                }
                            }
                                    ...
                                    if let indexPath = orderedIndexPaths.last {
                                        ...
                                        if indexPath.section == lastSection && indexPath.row == lastRow {
                                            if let cell = collectionView.cellForItem(at: indexPath) {
                                                self.frame = CGRect.init(x: 0, y: cell.frame.maxY, width: kScreenWitdh, height: 50)
                                            }
                                        }
                                    }
                                }
    ...
    func updateFrame() {
        if (superView?.isKind(of: UITableView.classForCoder()))! {
            if let tableView = superView as? UITableView {
                ...
                self.frame = CGRect.init(x: 0, y: y, width: kScreenWitdh, height: 50)
            }
        }
