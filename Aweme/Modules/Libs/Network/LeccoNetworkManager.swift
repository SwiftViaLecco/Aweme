...
class LeccoNetworkManager: NSObject {
...
extension LeccoNetworkManager {
    static func processData(data:[String:Any], success:@escaping HttpSuccess, failure:@escaping HttpFailure) {
        ...
        if(code == 0) {
            ...
            let error = NSError.init(domain: LeccoNetworkDomain, code: LeccoNetworkError.httpResquestFailed.rawValue, userInfo: [NSLocalizedDescriptionKey : message])
            failure(error)
        }
    }
    
    static func getRequest(urlPath:String, request:LeccoBaseRequest, success:@escaping HttpSuccess, failure:@escaping HttpFailure) {
        let parameters = request.toJSON()
        sessionManager.request(LeccoBaseUrl + urlPath, method: HTTPMethod.get, parameters: parameters, encoding: URLEncoding.default, headers: nil)
    ...
    static func deleteRequest(urlPath:String, request:LeccoBaseRequest, success:@escaping HttpSuccess, failure:@escaping HttpFailure){
        let parameters = request.toJSON()
        sessionManager.request(LeccoBaseUrl + urlPath, method: HTTPMethod.delete, parameters: parameters, encoding: URLEncoding.default, headers: nil)
            .validate(statusCode: 200..<300)
            .validate(contentType: ["application/json"])
            .responseJSON(completionHandler: { response in
                switch response.result {
    ...
    static func postRequest(urlPath:String, request:LeccoBaseRequest, success:@escaping HttpSuccess, failure:@escaping HttpFailure) {
        let parameters = request.toJSON()
        sessionManager.request(LeccoBaseUrl + urlPath, method: HTTPMethod.post, parameters: parameters, encoding: URLEncoding.default, headers: nil)
            .validate(statusCode: 200..<300)
            .validate(contentType: ["application/json"])
            .responseJSON(completionHandler: { response in
                switch response.result {
    ...
    static func uploadRequest(urlPath:String, data:Data, request:LeccoBaseRequest, progress:@escaping UploadProgress, success:@escaping HttpSuccess, failure:@escaping HttpFailure) {
        let parameters = request.toJSON()
        ...
        }, usingThreshold: UInt64.init(), to: LeccoBaseUrl + urlPath,
